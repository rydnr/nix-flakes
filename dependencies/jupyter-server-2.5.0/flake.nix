{
  description = "A Nix flake for jupyter-server 2.5.0 Python package";
  ...
  outputs = inputs:
    ...
    flake-utils.lib.eachDefaultSystem (system:
      ...
      in rec {
        packages = {
          jupyter-server-2.5.0 = (import ./jupyter-server-2.5.0.nix) {
            inherit (pythonPackages)
              buildPythonPackage fetchPypi
             inherit (pkgs) lib;
          };
          jupyter-server = packages.jupyter-server-2.5.0;
          default = packages.jupyter-server;
          meta = with lib; {
            description = ''
# Jupyter Server
