{
  description = "A Nix flake for deeplake 3.2.21 Python package";
  ...
  outputs = inputs:
    ...
    flake-utils.lib.eachDefaultSystem (system:
      ...
      in rec {
        packages = {
          deeplake-3.2.21 = (import ./deeplake-3.2.21.nix) {
            inherit (pythonPackages)
              buildPythonPackage fetchPypi
             inherit (pkgs) lib;
          };
          deeplake = packages.deeplake-3.2.21;
          default = packages.deeplake;
          meta = with lib; {
            description = ''
