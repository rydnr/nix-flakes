{
  description = "A Nix flake for manifest-ml 0.0.1 Python package";
  ...
  outputs = inputs:
    ...
    flake-utils.lib.eachDefaultSystem (system:
      ...
      in rec {
        packages = {
          manifest-ml-0.0.1 = (import ./manifest-ml-0.0.1.nix) {
            inherit (pythonPackages)
              buildPythonPackage fetchPypi
             inherit (pkgs) lib;
          };
          manifest-ml = packages.manifest-ml-0.0.1;
          default = packages.manifest-ml;
          meta = with lib; {
            description = ''
