{
  description = "A Nix flake for wikipedia 1.4.0 Python package";
  ...
  outputs = inputs:
    ...
    flake-utils.lib.eachDefaultSystem (system:
      ...
      in rec {
        packages = {
          wikipedia-1.4.0 = (import ./wikipedia-1.4.0.nix) {
            inherit (pythonPackages)
              buildPythonPackage fetchPypi
             inherit (pkgs) lib;
          };
          wikipedia = packages.wikipedia-1.4.0;
          default = packages.wikipedia;
          meta = with lib; {
            description = ''
Wikipedia
=========
