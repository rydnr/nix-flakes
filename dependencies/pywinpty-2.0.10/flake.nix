{
  description = "A Nix flake for pywinpty 2.0.10 Python package";
  ...
  outputs = inputs:
    ...
    flake-utils.lib.eachDefaultSystem (system:
      ...
      in rec {
        packages = {
          pywinpty-2.0.10 = (import ./pywinpty-2.0.10.nix) {
            inherit (pythonPackages)
              buildPythonPackage fetchPypi
             inherit (pkgs) lib;
          };
          pywinpty = packages.pywinpty-2.0.10;
          default = packages.pywinpty;
          meta = with lib; {
            description = ''
